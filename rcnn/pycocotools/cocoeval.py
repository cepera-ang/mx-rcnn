            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                        ...
                        nd = len(tp)
                        rc = old_div(tp, npig)
                        pr = old_div(tp, (fp+tp+np.spacing(1)))
                        q  = np.zeros((R,))
...
class Params(object):
    ...
    def __init__(self):
        ...
        self.iouThrs = np.linspace(.5, 0.95, np.round(old_div((0.95-.5),.05))+1, endpoint=True)
        self.recThrs = np.linspace(.0, 1.00, np.round(old_div((1.00-.0),.01))+1, endpoint=True)
